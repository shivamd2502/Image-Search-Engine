...
import math
import numpy as np
from embedding_manager import EmbeddingManager, start_watching
...
class ImageSearchApp(ctk.CTk):
    ...
    def create_header(self, parent):
        ...
        subtitle_label = ctk.CTkLabel(
            title_frame,
            text="search image by text query • optimized with embeddings cache",
    ...
    def create_sidebar(self, parent):
        ...
        self.embedding_status_frame = ctk.CTkFrame(folder_frame, fg_color="transparent")
        self.embedding_status_frame.pack(pady=(5, 0), fill="x")
        
        self.embedding_status_label = ctk.CTkLabel(
            self.embedding_status_frame,
            text="",
            font=ctk.CTkFont(size=10),
            text_color="gray"
        )
        self.embedding_status_label.pack(side="left")
    ...
    def create_advanced_settings(self, parent):
        ...
        self.rebuild_btn.pack(pady=(10, 15), padx=15, fill="x")
    ...
    def show_welcome_message(self):
        ...
        welcome_subtitle = ctk.CTkLabel(
            welcome_frame,
            text="Select a folder and describe what you're looking for\nOptimized with embedding cache for lightning-fast searches ⚡",
            font=ctk.CTkFont(size=14),
            text_color="gray"
        )
    ...
    def update_header_stats(self):
        ...
        if self.folder_path and self.embedding_manager:
            ...
            stats_label = ctk.CTkLabel(
                self.stats_frame,
                text=f"📁 {image_count} images cached • 🔍 {len(self.search_history)} searches",
                font=ctk.CTkFont(size=12),
                text_color="gray"
            )
            stats_label.pack()
    ...
    def select_folder(self):
        ...
        if folder_path:
            ...
            self.initialize_embeddings(folder_path)

    def initialize_embeddings(self, folder_path):
        """Initialize the embedding manager and load/compute embeddings."""
        self.embedding_status_label.configure(
            text="⏳ Initializing embeddings...",
            text_color="orange"
        )
        self.status_label.configure(text="Loading embedding cache...")
        
        # Run in background thread
        threading.Thread(
            target=self._load_embeddings_thread,
            args=(folder_path,),
            daemon=True
        ).start()

    def _load_embeddings_thread(self, folder_path):
        """Background thread to load/compute embeddings."""
        try:
            ...
            cache_path = os.path.join(folder_path, "embeddings_cache.pkl")
            ...
            # Update cache (compute new embeddings)
            self.after(0, lambda: self.status_label.configure(
                text="Computing embeddings for new images..."
            ))
            
            self.embedding_manager.update_cache()
            
            # Start watching folder for changes
            self.folder_observer = start_watching(folder_path, self.embedding_manager)
            
            # Update UI
            embeddings = self.embedding_manager.get_embeddings()
            self.embeddings_loaded = True
            
            self.after(0, lambda: self._embeddings_loaded_callback(len(embeddings)))
    ...
    def _embeddings_loaded_callback(self, count):
        """Called when embeddings are loaded."""
        self.embedding_status_label.configure(
            text=f"✅ {count} images cached • Auto-updating",
            text_color=self.colors['success']
        )
        self.status_label.configure(
            text=f"Embeddings loaded: {count} images ready for instant search!"
        )
        self.update_header_stats()

    def rebuild_embeddings(self):
        """Rebuild the entire embeddings cache."""
        ...
        confirm = CTkMessagebox(
            title="Rebuild Cache?",
            message="This will recompute all embeddings. Continue?",
            icon="question",
            option_1="Cancel",
            option_2="Rebuild"
        )
        
        if confirm.get() == "Rebuild":
            # Delete existing cache
            cache_path = os.path.join(self.folder_path, "embeddings_cache.pkl")
            if os.path.exists(cache_path):
                os.remove(cache_path)
            
            # Reinitialize
            self.initialize_embeddings(self.folder_path)
    ...
    def start_search(self):
        ...
        if not self.embeddings_loaded:
            CTkMessagebox(
                title="Embeddings Loading",
                message="Please wait for embeddings to finish loading!",
                icon="warning"
            )
            return
        ...
        threading.Thread(
            target=lambda: self.search_images_optimized(prompt),
            daemon=True
        ).start()
    ...
    def search_images_optimized(self, prompt):
        """Optimized search using pre-computed embeddings."""
        try:
            ...
            text_features = text_features / np.linalg.norm(text_features)  # Normalize
            
            # Get cached embeddings
            self.after(0, lambda: self.status_label.configure(text="Comparing with cached embeddings..."))
            cached_embeddings = self.embedding_manager.get_embeddings()
            
            # Compute similarities
            results = []
            threshold = self.threshold_slider.get()
            
            for img_path, img_embedding in cached_embeddings.items():
                # Compute cosine similarity
                similarity = float(np.dot(text_features.flatten(), img_embedding.flatten()))
    ...
    def show_search_results(self, results, search_time, prompt):
        ...
        stats_text = f"⚡ Lightning-fast search: {search_time:.3f}s • 🎯 Best match: {results[0][1]:.3f}"
        ctk.CTkLabel(
            stats_frame,
            text=stats_text,
            font=ctk.CTkFont(size=12),
        ...
        self.status_label.configure(text=f"Search completed: {len(results)} results in {search_time:.3f}s")
